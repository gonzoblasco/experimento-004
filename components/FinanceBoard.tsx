...
function calcTotals(entries: FinanceEntry[], start: Date) {
  return entries
    .filter((entry) => new Date(entry.occurredOn) >= start)
    .reduce(
      (acc, entry) => {
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-sm">
              ...
              <input
                ...
                onChange={(event) =>
                  setDraft((prev) => ({ ...prev, occurredOn: event.target.value }))
                }
              />
            </label>
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {new Date(entry.occurredOn).toLocaleDateString()} · {entry.type.toLowerCase()}
          </p>
